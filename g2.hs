...
updateGeneration g as = take popSize $ cycle newPop
    where popSize = length as
          ...
          newPop = take somme children ++ chosen ++ drop somme children ++ mutants
          mutants = []
          somme = 10

top5 pop = take 5 $ map fitness $ orderByFitness fitness pop

generations :: RandomGen g => g -> [[GenomeElem]] ->  [ [[GenomeElem]] ]
generations g x = y:generations g2 y
    where (g1,g2) = split g
          y = updateGeneration g1 x
